...
// Fetch user data
$user_stmt = $conn->prepare("SELECT user_id, full_name, email, img_name, role, created_at FROM users WHERE user_id = ?");
...
$groups_stmt = $conn->prepare("
    SELECT gm.member_id, gm.group_id, gm.member_role, g.group_name, g.group_type 
...
$user_groups = $groups_result->fetch_all(MYSQLI_ASSOC);
$groups_stmt->close();

// Check if user has no groups and redirect
if (empty($user_groups)) {
    $_SESSION['alert'] = [
        'type' => 'info',
        'message' => 'Please create or join a group first to access your profile.'
    ];
    header("Location: " . $baseUrl . "/user/customer/groups/create_group.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_member_role'])) {
    ...
    $new_role = $_POST['member_role'];
    $group_type = $_POST['group_type'];
    
    // Define valid roles based on group type
    $valid_roles_by_type = [
        'household' => ['parent', 'child', 'other'],
        'co_living' => ['member'],
        'small_business' => ['manager', 'staff']
    ];
    
    // Validate role based on group type
    if (!isset($valid_roles_by_type[$group_type]) || !in_array($new_role, $valid_roles_by_type[$group_type])) {
        $error = "Invalid member role for this group type";
    } else {
        // Verify that this member_id belongs to the current user
        $verify_stmt = $conn->prepare("SELECT member_id FROM group_members WHERE member_id = ? AND user_id = ?");
        $verify_stmt->bind_param("ii", $member_id, $user_id);
        $verify_stmt->execute();
        $verify_result = $verify_stmt->get_result();
        
        if ($verify_result->num_rows === 0) {
            $error = "Invalid member record";
        } else {
            // Update the member role
            $update_role_stmt = $conn->prepare("UPDATE group_members SET member_role = ? WHERE member_id = ?");
            $update_role_stmt->bind_param("si", $new_role, $member_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['update_member_role'])) {
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        // Check if form fields are present (not just picture upload without file)
        if (isset($_POST['full_name']) && isset($_POST['email'])) {
            ...
            // No file uploaded and no form data - likely accidental button click
            $error = "Please select a file to upload or use the form below to update your information.";
        }
    }
}
    ...
    <div class="profile-container">
        <div class="profile-header">
            ...
            <div class="profile-actions">
                <a href="<?= htmlspecialchars($baseUrl) ?>/user/profile/profile.php" class="btn btn-delete">
        ...
        <div class="orders-section" style="margin-bottom: 30px;">
            ...
            <div style="display: grid; gap: 20px;">
                <?php foreach ($user_groups as $group): 
                    // Define role options based on group type
                    $role_options = [];
                ...
                <div class="info-card">
                    <form method="POST" action="">
                        ...
                        <input type="hidden" name="member_id" value="<?php echo $group['member_id']; ?>">
                        <input type="hidden" name="group_type" value="<?php echo htmlspecialchars($group['group_type']); ?>">
                        
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 20px; align-items: end;">
                            <div>
                                <label style="display: block; font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; color: rgba(0,0,0,0.6); margin-bottom: 10px; font-weight: 600;">
                                    Group Name
                                </label>
                                <div style="padding: 14px 18px; background: #f5f5f5; border: 1px solid rgba(0,0,0,0.06); font-size: 14px; color: #0a0a0a;">
                            ...
                            <div>
                                ...
                                <div style="padding: 14px 18px; background: #f5f5f5; border: 1px solid rgba(0,0,0,0.06); font-size: 14px; color: #0a0a0a; text-transform: capitalize;">
                                    <?php echo htmlspecialchars(str_replace('_', ' ', $group['group_type'])); ?>
                                </div>
                            </div>
                            
                            <div>
                                <label for="member_role_<?php echo $group['member_id']; ?>" style="display: block; font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; color: rgba(0,0,0,0.6); margin-bottom: 10px; font-weight: 600;">
                                    Member Role
                                </label>
                                <select id="member_role_<?php echo $group['member_id']; ?>" 
                                        name="member_role"
                                        style="width: 100%; padding: 14px 18px; font-size: 14px; font-family: 'Montserrat', sans-serif; background: #fafafa; border: 1px solid rgba(0,0,0,0.1); transition: all 0.3s ease; cursor: pointer;">
                                    <?php foreach ($role_options as $value => $label): ?>
                                        <option value="<?php echo htmlspecialchars($value); ?>" 
                                                <?php echo $group['member_role'] === $value ? 'selected' : ''; ?>>
