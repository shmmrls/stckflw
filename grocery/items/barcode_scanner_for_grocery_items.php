...
// Get user's store_id
$user_stmt = $conn->prepare("SELECT store_id, role FROM users WHERE user_id = ?");
$user_stmt->bind_param("i", $user_id);
$user_stmt->execute();
$user_result = $user_stmt->get_result();
$user_data = $user_result->fetch_assoc();

if ($user_data['role'] !== 'grocery_admin' || !$user_data['store_id']) {
    die("Access denied. Only grocery admins with assigned stores can access this page.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['barcode'])) {
    ...
    // First, check if item already exists in grocery_items for this store
    $existing_stmt = $conn->prepare("
        SELECT gi.*, c.category_name 
        FROM grocery_items gi
        LEFT JOIN categories c ON gi.category_id = c.category_id
        WHERE gi.barcode = ? AND gi.store_id = ?
        ORDER BY gi.expiry_date ASC
        LIMIT 1
    ");
    $existing_stmt->bind_param("si", $barcode, $store_id);
    ...
    if ($existing_result->num_rows > 0) {
        ...
        $log_stmt = $conn->prepare("
            ...
            VALUES (?, ?, 'lookup', 1, ?)
        ");
        $log_stmt->bind_param("isi", $user_id, $barcode, $existing_item['item_id']);
        $log_stmt->execute();
    ...
    // If not in inventory, look up in product catalog
    $stmt = $conn->prepare("
    ...
    <div class="scanner-container">
        ...
        <div class="page-header">
            <div class="header-content">
                <div class="header-info">
                    ...
                    <p class="page-subtitle">Scan products to quickly add them to your store inventory</p>
                </div>
        ...
        <div id="product-result" class="product-result" style="display: none;">
            ...
            <div class="product-details">
                ...
                <div class="detail-row" id="existing-quantity-row" style="display: none;">
                    <span class="detail-label">Current Quantity</span>
                    <span class="detail-value" id="product-quantity"></span>
                </div>
                <div class="detail-row" id="existing-expiry-row" style="display: none;">
                    <span class="detail-label">Expiry Date</span>
                    <span class="detail-value" id="product-expiry"></span>
                </div>
                <div class="detail-row">
            ...
            <div class="result-actions">
                <button id="add-to-inventory" class="btn-success">
                    ...
                    <span id="add-btn-text">Add to Inventory</span>
                </button>
...
<script>
    ...
    function lookupBarcode(barcode) {
        fetch('barcode_scanner_for_grocery_items.php', {
            method: 'POST',
        ...
        .then(data => {
            if (data.success) {
                ...
                isExistingItem = data.existing || false;
                
                displayProduct(data.product, barcode, isExistingItem);
                
                if (isExistingItem) {
                    showSuccess('Barcode: ' + barcode + ' - Item already in your inventory!');
                } else {
                    showSuccess('Barcode: ' + barcode + ' - Product found in catalog!');
                }
                stopCamera();
            } else {
    ...
    function displayProduct(product, barcode, existing) {
        document.getElementById('product-name').textContent = product.product_name || product.item_name;
        ...
        document.getElementById('product-unit').textContent = product.default_unit || product.unit;
        ...
        if (existing) {
            document.getElementById('result-title').textContent = 'Item Already in Inventory!';
            document.getElementById('existing-quantity-row').style.display = 'flex';
            document.getElementById('existing-expiry-row').style.display = 'flex';
            document.getElementById('product-quantity').textContent = product.quantity + ' ' + product.unit;
            document.getElementById('product-expiry').textContent = product.expiry_date;
            document.getElementById('add-btn-text').textContent = 'Update Stock';
        } else {
            document.getElementById('result-title').textContent = 'Product Found in Catalog!';
            document.getElementById('existing-quantity-row').style.display = 'none';
            document.getElementById('existing-expiry-row').style.display = 'none';
            document.getElementById('add-btn-text').textContent = 'Add to Inventory';
        }
    ...
    document.getElementById('add-to-inventory').addEventListener('click', function() {
        if (currentProduct) {
            const params = new URLSearchParams({
                ...
                item_name: currentProduct.product_name || currentProduct.item_name || '',
                category_id: currentProduct.category_id || '',
                catalog_id: currentProduct.catalog_id || '',
                unit: currentProduct.default_unit || currentProduct.unit || 'pcs',
                shelf_life: currentProduct.typical_shelf_life_days || ''
            });
            
            if (isExistingItem) {
                params.append('item_id', currentProduct.item_id);
            }
